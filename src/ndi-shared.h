...
// RESPONSE_FRAME_SIZE previously doubled as VIDEO_FRAME_OFFSET, but it only accounted for
// the two raw struct sizes (72+64=136 bytes) and not the 4-byte NDI_*_FRAME type marker
// serialize_frame()/deserialize_frame() prepend to *each* of them (audio: 4+64, video:
// 4+72 = 144 bytes total) - so the frame-sync wire format's video struct (whose last 8
// bytes are its timestamp field) actually extended 8 bytes past where VIDEO_FRAME_OFFSET
// began. Since the custom video/audio allocators are registered on every receiver
// unconditionally (used for the non-frame-sync NDI_CAPTURE_FRAME path) and point
// mem_block->video_ptr/audio_ptr at exactly VIDEO_FRAME_OFFSET/AUDIO_FRAME_OFFSET, real
// incoming pixel bytes were landing directly on top of the frame-sync video struct's
// timestamp field - not corruption, but two unrelated regions that were never supposed to
// overlap sharing the same starting address. Give the header region a generous, clearly
// separate size so this can't recur even if the structs grow.
static const DWORD RESPONSE_FRAME_SIZE = sizeof(NDIlib_video_frame_v2_t) + +sizeof(NDIlib_audio_frame_v3_t);
static const DWORD RESPONSE_HEADER_SIZE = 4096u; // >> 2*sizeof(uint32_t) + RESPONSE_FRAME_SIZE (144 bytes today)
static const DWORD VIDEO_FRAME_OFFSET = RESPONSE_HEADER_SIZE;
static const DWORD AUDIO_FRAME_OFFSET = VIDEO_FRAME_OFFSET + VIDEO_FRAME_MAX_SIZE;
static const DWORD RESPONSE_SIZE = sizeof(uint32_t) + RESPONSE_HEADER_SIZE + SHARED_MEM_SIZE;
				  ...
				  std::vector<std::string> &out_strings)
{
	...
	// NDIlib_recv_color_format_e is the same type on both sides of this wire format:
	// Processing.NDI.Advanced.h (used here, on the server) includes Processing.NDI.Lib.h
	// (used by the client), so the enum values line up and this cast is safe.
	out_desc.color_format = static_cast<NDIlib_recv_color_format_e>(cf_u32);
