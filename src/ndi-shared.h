...
static const DWORD VIDEO_FRAME_MAX_SIZE = 4000u * 3000u * 4u;
static const DWORD SHARED_MEM_SIZE = VIDEO_FRAME_MAX_SIZE + AUDIO_FRAME_MAX_SIZE;
static const DWORD RESPONSE_FRAME_SIZE =
	sizeof(uint32_t) + sizeof(NDIlib_video_frame_v2_t) + sizeof(uint32_t) + sizeof(NDIlib_audio_frame_v3_t);
...
static const DWORD REQUEST_FRAME_SIZE =
	sizeof(uint32_t) + sizeof(NDIlib_video_frame_v2_t) + sizeof(uint32_t) + sizeof(NDIlib_audio_frame_v3_t);
static const DWORD REQUEST_SIZE = sizeof(uint32_t) + sizeof(uint32_t) + REQUEST_FRAME_SIZE + SHARED_MEM_SIZE;
...
// Returns the byte stride to use per-row for the given NDI video FourCC and horizontal resolution.
// The caller can multiply the returned stride by `yres` to get the total `p_data` size used elsewhere
// in this codebase (serialize_frame / deserialize_frame expect data_size_in_bytes * yres).
//
// Notes:
// - For packed 4-byte formats (BGRA, RGBA, etc.) stride = xres * 4.
// - For YUY2/UYVY packed 16-bit formats stride = xres * 2.
// - For common 4:2:0 planar formats (I420, NV12) we return xres * 3/2 so that
//   stride * yres == total bytes (Y plane + UV planes combined).
// - For unknown/unsupported FourCC we fall back to 4 bytes/pixel.
// - This function intentionally returns 0 for non-positive xres.
static inline size_t
video_frame_row_stride(NDIlib_FourCC_video_type_e fourcc, int xres)
{
	if (xres <= 0)
		return 0;

	const size_t ux = static_cast<size_t>(xres);
	...
	case NDIlib_FourCC_type_UYVA:
		// 2 bytes per pixel (packed YUV 4:2:2)
		return ux * 2u;

	case NDIlib_FourCC_type_I420:
	case NDIlib_FourCC_type_NV12:
		// Planar 4:2:0 formats: total size = xres * yres * 3 / 2
		// We return bytes-per-row such that (stride * yres) == total size.
		// Use integer arithmetic: (xres * 3) / 2
		return (ux * 3u) / 2u;

		// Add other FourCC cases here if required by your project.

	default:
		// Conservative fallback: assume 4 bytes per pixel
		return ux * 4u;
	}
}
//
			      ...
			      bool copy_data = false)
{
	...
	if (frame_type == NDIlib_frame_type_video) {
		...
		if (copy_data) {
			// Determine data size
			size_t data_size = video_frame_row_stride(vf->FourCC, vf->xres) * vf->yres;

			// Check total space
			if (remaining < struct_sz + data_size)
				return 0;

			// append frame data
			if (data_size > 0) {
				if (!write_bytes(vf->p_data, data_size))
					return 0;
			}
		}
...
// Serialize NDIlib_send_create_t into a byte buffer suitable for passing via shared memory.
// Format:
// [uint8_t clock_video][uint8_t clock_audio][uint32_t p_ndi_name_len][uint32_t p_groups_len]
// [p_ndi_name bytes][p_groups bytes]
// All string lengths are byte counts (no terminating NUL included). If a string is NULL, length==0 and no bytes are written.
static bool serialize_send_desc(const NDIlib_send_create_t &desc, void *out_buf, size_t out_size, size_t &out_written)
{
	if (!out_buf || out_size < sizeof(uint32_t) * 2 + sizeof(uint8_t) * 2)
		return false;
	...
	const char *name = desc.p_ndi_name ? desc.p_ndi_name : "";
	const char *groups = desc.p_groups ? desc.p_groups : "";

	uint32_t name_len = static_cast<uint32_t>(strlen(name));
	uint32_t groups_len = static_cast<uint32_t>(strlen(groups));

	uint8_t clock_video = desc.clock_video ? 1 : 0;
	uint8_t clock_audio = desc.clock_audio ? 1 : 0;

	// write booleans
	if (!write_u8(clock_video))
		return false;
	if (!write_u8(clock_audio))
		return false;

	// write lengths
	if (!write_u32(name_len))
		return false;
	if (!write_u32(groups_len))
		return false;
	...
	if (!write_bytes(name, name_len))
		return false;
	if (!write_bytes(groups, groups_len))
		return false;
...
// Deserialize buffer to NDIlib_send_create_t. The function fills out_desc and returns a vector of strings that own the memory
// pointed to by out_desc (the caller must keep the returned vector alive while using out_desc pointers).
static bool deserialize_send_desc(const void *buf, size_t buf_len, NDIlib_send_create_t &out_desc,
				  std::vector<std::string> &out_strings)
{
	if (!buf || buf_len < sizeof(uint32_t) * 2 + sizeof(uint8_t) * 2)
		return false;
	...
	uint8_t clock_video = 0;
	uint8_t clock_audio = 0;
	uint32_t name_len = 0;
	uint32_t groups_len = 0;

	if (!read_u8(clock_video))
		return false;
	if (!read_u8(clock_audio))
		return false;
	if (!read_u32(name_len))
		return false;
	if (!read_u32(groups_len))
		return false;

	// Validate lengths
	size_t total_str_len = static_cast<size_t>(name_len) + static_cast<size_t>(groups_len);
	if (total_str_len > remaining)
		return false;

	out_strings.clear();
	out_strings.reserve(2);

	if (name_len > 0) {
		out_strings.emplace_back(reinterpret_cast<const char *>(p), name_len);
	} else {
		out_strings.emplace_back();
	}
	p += name_len;
	...
	if (groups_len > 0) {
		out_strings.emplace_back(reinterpret_cast<const char *>(p), groups_len);
	} else {
		out_strings.emplace_back();
	}
	p += groups_len;
	remaining -= groups_len;

	// Fill out_desc
	memset(&out_desc, 0, sizeof(out_desc));
	out_desc.p_ndi_name = out_strings[0].empty() ? nullptr : out_strings[0].c_str();
	out_desc.p_groups = out_strings[1].empty() ? nullptr : out_strings[1].c_str();
	out_desc.clock_video = (clock_video != 0);
	out_desc.clock_audio = (clock_audio != 0);
