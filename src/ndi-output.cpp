...
void ndi_output_update(void *data, obs_data_t *settings)
{
	...
	if (outputName && std::strcmp(outputName, "NDI Main Output") == 0) {
		// Don't save the temporary test output name used by main_output_is_supported()
		// (e.g. "NDI Output Support Test 12345"). Only persist real configured names.
		const char *support_test_prefix = "NDI Output Support Test";
		bool is_support_test_name = false;
		if (o->ndi_name) {
			is_support_test_name =
				(std::strncmp(o->ndi_name, support_test_prefix, std::strlen(support_test_prefix)) == 0);
		}
		if (!is_support_test_name) {
