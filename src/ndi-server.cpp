...
int main(int argc, char *argv[])
{
	...
	} else if (!ndi->initialize()) {
		// The NDI SDK requires NDIlib_initialize() to run before any other NDI call - skipping
		// it left recv_create_v3()/framesync_create() operating on an uninitialized runtime,
		// which is what was actually causing NDI_CREATE_RECEIVER to never come back with a
		// response (the client-visible symptom was just "times out waiting for a response").
		printf("Warning: NDI runtime initialize() failed (unsupported CPU?). ndi-server may not function correctly.\n");
	} else {
		printf("NDI runtime initialized.\n");
	}
	...
	// malloc() does not zero-initialize: n_ptrs/next_video_ptr/next_audio_ptr were
	// being left as garbage, and audio_custom_allocator() below divides by n_ptrs
	// (`next_audio_ptr = (next_audio_ptr + 1) % n_ptrs`) - whenever that garbage
	// happened to be 0, the first FLTP audio frame crashed this process outright,
	// which is exactly what made some sources silently stop receiving frames
	// while others (with luckier garbage) kept working.
	mem_block->n_ptrs = 1;
	mem_block->next_video_ptr = 0;
	mem_block->next_audio_ptr = 0;
	...
	while (g_running) {
		...
		case NDI_CREATE_RECEIVER: {
			...
			// p_ndi_name is null whenever the client's source name was empty (e.g. no NDI
			// source picked yet): deserialize_recv_desc() maps an empty string to nullptr,
			// and passing that straight to printf's %s crashes the UCRT release runtime.
			printf("Received ndi name: '%s'\n",
			       recv_desc.source_to_connect_to.p_ndi_name ? recv_desc.source_to_connect_to.p_ndi_name
									  : "(none)");
			if (ndi_receiver) {
		...
		case NDI_CAPTURE_FRAME: {
			...
			case NDIlib_frame_type_audio:
				...
				break;
			default:
				// No frame within the capture timeout: without this, payload_type keeps
				// whatever it was left at by the previous response and the client can
				// misread stale video/audio bytes as a brand-new frame.
