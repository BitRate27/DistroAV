...
void obs_sync_debug_log(const char *message, int64_t timestamp, int64_t *audio_on_time,
			int64_t *audio_off_time, int audio_sync_count, int64_t *white_on_time,
			int64_t *white_off_time, int video_sync_count)
{
	if (timestamp > std::max<int64_t>(*audio_on_time, *white_on_time) + max_offset) {
		if (*white_on_time > 0 && *audio_on_time > 0 && audio_sync_count > 0 && video_sync_count > 0) {
			...
			obs_log(LOG_DEBUG,
				...
				*audio_off_time > 0 ? (*audio_off_time - *audio_on_time) / 1000000 : -1, audio_sync_count,
				*white_on_time / 1000000,
				*white_off_time > 0 ? (*white_off_time - *white_on_time) / 1000000 : -1, video_sync_count,
				diff / 1000000);
			*audio_on_time = 0;
			*audio_off_time = 0;
			*white_on_time = 0;
...
void obs_sync_debug_log_video_time(const char *message, const char *source_ndi_name, uint64_t timestamp, uint8_t *data)
{
	...
	obs_sync_debug_log(message, timestamp, audio_on_time, audio_off_time, *audio_sync_count, white_on_time,
