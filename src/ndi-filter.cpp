...
void ndi_filter_raw_video(void *data, video_data *frame)
{
	...
	if (NDIAdvlib)
		NDIAdvlib->send_send_video_v2(f->ndi_sender, &video_frame);
	else if (ndiLib)
...
void ndi_sender_destroy(ndi_filter_t *filter)
{
	...
	if (NDIAdvlib)
		NDIAdvlib->send_destroy(filter->ndi_sender);
	else if (ndiLib)
...
void ndi_sender_create(ndi_filter_t *filter, obs_data_t *settings)
{
	...
	if (NDIAdvlib)
		filter->ndi_sender = NDIAdvlib->send_create_v2(&send_desc, nullptr);
	else if (ndiLib)
...
void ndi_filter_destroy(void *data)
{
	...
	if (NDIAdvlib)
		NDIAdvlib->send_destroy(f->ndi_sender);
	else if (ndiLib) {
...
obs_audio_data *ndi_filter_asyncaudio(void *data, obs_audio_data *audio_data)
{
	...
	pthread_mutex_lock(&f->ndi_sender_audio_mutex);
	if (NDIAdvlib)
		NDIAdvlib->send_send_audio_v3(f->ndi_sender, &audio_frame);
	else if (ndiLib) {
