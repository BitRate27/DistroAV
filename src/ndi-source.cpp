...
void *ndi_source_thread(void *data)
{
	...
	while (s->running) {
		...
		if (s->config.reset_ndi_receiver) {
			...
			// Store the ndi_recv_name for use when the recv is not being shown and metadata only is needed
			meta_recv_desc.p_ndi_recv_name = s->config.ndi_receiver_name;
			last_showing = true;
